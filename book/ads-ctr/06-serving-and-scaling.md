...
**Details.** The sharded-embedding-table pattern that keeps billions of ids in memory across hosts was established by DLRM (Meta, 2019): embedding tables are model-parallel (each host owns a shard) while the interaction MLP is data-parallel, so a single lookup fans out to an all-to-all exchange whose network cost is the real p99 driver, not the arithmetic. On the calibration-drift row, the decoupled recalibration layer is a Platt scaling (Platt, 1999) or isotonic fit refit on fresh held-out data; it is cheap because it fits only one or two parameters over predicted-probability buckets, so it can run hourly while the full network retrains daily.
