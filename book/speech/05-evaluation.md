...
Word error rate is the standard:

$$\text{WER} = \frac{S + I + D}{N}$$

where $S$, $I$, $D$ count substituted, inserted, and deleted words against the
reference, and $N$ is the reference word count. Character error rate (CER) swaps
words for characters and is the fairer cross-language signal, since morphologically
rich languages produce long words that WER penalizes as multiple substitutions.
...
- **Real-time factor (RTF)**: $\text{RTF} = T_{\text{compute}} / T_{\text{audio}}$.
  RTF below 1.0 is required for streaming on a single CPU core; RTF below 0.1 is
  needed for headroom on a mobile device.
...
- **False reject rate (FRR)**: the fraction of real triggers missed.
- **Equal error rate (EER)**: the operating point where FAR equals FRR. Apple
  reports a 4.3% EER for personalized Hey Siri. EER summarizes the DET curve in
  a single number.
...
**mean opinion score (MOS)**: human raters listen to synthesized speech and score
naturalness from 1 (bad) to 5 (excellent). Near-human TTS reaches MOS around 4.5.

Automatic metrics (mel-cepstral distortion, PESQ) correlate poorly with perceived
naturalness and should not be the primary signal. Use them as fast regression
...
Diarization is scored by **diarization error rate (DER)**:
...
$T_{\text{false}}$ is time where a non-speaker is labeled, and
$T_{\text{confusion}}$ is time where a speaker is labeled as the wrong person.
Report purity (each cluster is dominated by one speaker) and coverage (each
speaker's turns are captured) alongside DER to diagnose the error composition.
