...
| Pinterest GPU serving | GPU fleet, custom batching | Large GPU batch, CUDA graphs, single device copy | (not documented in first-party source) | Centralized GPU inference | Moved from CPU scatter-gather to GPU to serve a much larger model at neutral cost; hybrid DRAM plus SSD embedding cache |
