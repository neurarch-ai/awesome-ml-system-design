...
bank of ~172 rapid-fire, depth-probing questions: the follow-ups an interviewer
pulls once the design is on the board (BatchNorm versus LayerNorm, calibration and the right metric, target leakage, classical models, ensembles, statistics, and loss functions, and which architecture moves which metric. All formulas now render as LaTeX). Each answer leads with the mechanism, then the tradeoff.
