...
| Should we replace our recommendation cascade with one generative model? | Item representation (atomic, semantic ID, text), the vocabulary collapse, decode cost against an ANN probe, what the cascade still owes | [19 generative recommendation](topics/19-generative-recommendation.md), [01 retrieval](topics/01-candidate-retrieval.md), [02 ranking](topics/02-ranking-model.md) |
| Our new content type has no features and no interaction history. How would you recommend it? | Content-derived representations, cold-start generalization, an LLM in the loop, and the engineering-velocity argument | [19 generative recommendation](topics/19-generative-recommendation.md), [18 cold start](topics/18-cold-start-and-exploration.md) |
